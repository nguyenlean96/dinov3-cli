...
def extract(
    ...
    pool: bool = typer.Option(
        False, 
        "--pool/--no-pool", 
        help="Whether to pool features into a single embedding"
    ),
    output: Optional[Path] = typer.Option(
        None, 
        "-o", "--output", 
        help="Output file (.npy or .json). Defaults to stdout as JSON if not provided."
    ),
    ...
    try:
        ...
        if output:
            if output.suffix == ".npy":
                ...
                console.print(f"[bold green]✓[/bold green] Features saved to [cyan]{output}[/cyan] with shape {features.shape}")
            elif output.suffix == ".json":
                with open(output, "w") as f:
                    json.dump(features.tolist(), f)
                console.print(f"[bold green]✓[/bold green] Features saved to [cyan]{output}[/cyan] with shape {features.shape}")
            else:
                console.print(f"[bold yellow]Warning:[/bold yellow] Unknown extension '{output.suffix}'. Defaulting to numpy format.")
                np.save(output, features)
                console.print(f"[bold green]✓[/bold green] Features saved to [cyan]{output}[/cyan] with shape {features.shape}")
        else:
